...
void sepia_filter_asm(image_t* img );
...
char* replace(char* arg){
	int i;
	/* char newstr[10]; */
	char* newstr = (char*) malloc(sizeof(char)*strlen(arg));
...
const char* invitation = "Your choice: "; /* prompt */
const char* invalid_input = "I don't know what you mean. Please try again.\n";
...
void main_menu()
{
	...
	printf("3 - Print\n");
	printf("4 - Sepia using C\n");
	printf("5 - Sepia using ASM\n");
	printf("6 - Luminosity\n");
	printf("7 - OpenCL\n");
}
void morph_menu()
...
int main/* _release */( int argc, char** argv ) {
	...
	switch (choice)
	{
		...
		case 7:
			...
			//clgreyscale(&img,&img);
			//cldilate(&img,&img);
			//clboarder(&img,&img, "sobel", 50);
			//colours2bin(&img,&img,170);
			//tophat(&img,&img);
...
int erode(image_t const * const source, image_t* dest )/*min val from neighborhood*/
 {
	...
	result.data=(pixel_t*)calloc(sizeof(pixel_t), result.width*result.height);
